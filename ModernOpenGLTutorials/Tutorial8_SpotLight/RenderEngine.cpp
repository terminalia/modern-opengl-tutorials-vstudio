...
void RenderEngine::initGL()
{
	...
	try {
		//SHADERS
		spotLightShader = new ShaderProgram("SpotLightShader", "../Shaders/Tutorial8/spot_light.vert", "../Shaders/Tutorial8/spot_light_outerCutOff.frag");
		uiShader = new ShaderProgram("uiShader", "../Shaders/Tutorial8/ui.vert", "../Shaders/Tutorial8/ui.frag");
		fboShader = new ShaderProgram("fboShader", "../Shaders/Tutorial8/fboscreen.vert", "../Shaders/Tutorial8/fboscreen.frag");
...
void RenderEngine::render()
{
	...
	spotLightShader->uploadMat4("uProjectionMat", camera.getProjectionMatrix());
	spotLightShader->uploadMat4("uViewMat", camera.getViewMatrix());
	spotLightShader->uploadVec3("uCameraPosition", camera.getPosition());
	
	//APPLY LIGTHING
	for (unsigned int i = 0; i < lights.size(); i++)
	{
		lights.at(i)->uploadUniformsToShader(spotLightShader);
	}

	//RENDER MODELS
	for (unsigned int i = 0; i < models.size(); i++)
	{
		spotLightShader->uploadMat4("uModelMat", models[i]->getModelMatrix());
		spotLightShader->uploadMat4("uNormalMat", models[i]->getNormalMatrix());
		models.at(i)->draw();
	}
